...
export function TeacherCard({ teacher }: TeacherCardProps) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-3xl border border-white/60 bg-[color:var(--surface)] p-6 shadow-[0_20px_60px_-28px_rgba(15,23,42,0.35)] transition duration-300 hover:-translate-y-0.5 hover:border-teal-200/80 hover:shadow-[0_28px_70px_-30px_rgba(13,148,136,0.45)]"
    >
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-br from-white/80 via-transparent to-teal-50/40 opacity-0 transition group-hover:opacity-100" />
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <span className="inline-flex h-7 min-w-[2rem] items-center justify-center rounded-lg bg-teal-100 px-2 text-xs font-bold tabular-nums text-teal-900 ring-1 ring-teal-200/80">
                  №{teacher.listNumber}
                </span>
                ...
                {teacher.offersConsultation ? (
                  <span className="shrink-0 rounded-md bg-violet-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-violet-900 ring-1 ring-violet-200/80">
                    Konsultatsiya
                  </span>
                ) : null}
              </div>
            ...
            <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-2xl bg-white/80 text-teal-700 shadow-sm ring-1 ring-black/5 transition group-hover:bg-teal-50">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {typeof teacher.experienceYears === "number" ? (
              <span className="inline-flex items-center rounded-full bg-amber-50/90 px-2.5 py-0.5 text-[11px] font-medium text-amber-900 ring-1 ring-amber-100">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <span
              className="inline-flex items-center gap-1.5 rounded-full bg-white/85 px-2.5 py-1 text-[11px] font-semibold text-teal-950 ring-1 ring-teal-100/90"
              title="Asosiy o‘qituvchi sifatida biriktirilgan o‘quvchilar"
            >
              <Users className="h-3.5 w-3.5 shrink-0 text-teal-600" aria-hidden />
              <span className="tabular-nums">{teacher._count.students}</span>
              <span className="font-medium text-teal-900/85">asosiy</span>
            </span>
            <span
              className="inline-flex items-center gap-1.5 rounded-full bg-white/85 px-2.5 py-1 text-[11px] font-semibold text-teal-950 ring-1 ring-teal-100/90"
            ...
            >
              ...
              <span className="font-medium text-teal-900/85">dars</span>
            </span>
          </div>
        </div>
      </div>
      {topSpecs.length > 0 ? (
        <ul className="relative mt-5 flex flex-wrap gap-2">
          {topSpecs.map((s) => (
            <li
              key={s}
              className="rounded-full bg-white/90 px-3 py-1 text-xs font-medium text-teal-900/90 ring-1 ring-teal-100"
            >
              {s}
            </li>
          ))}
      ...
      <div className="relative mt-6 space-y-2 border-t border-white/60 pt-4 text-sm text-[var(--muted)]">
        {teacher.phone ? (
          <p className="flex items-center gap-2 truncate">
            <Phone className="h-4 w-4 shrink-0 text-teal-700/80" aria-hidden />
